...
class ConfigurationManager {
    constructor(context) {
        ...
        this._useManifestConfig = 'target.useManifest';
        this._installDependenciesConfig = 'target.installDependencies';
        this._preferSystemLibsConfig = 'target.preferSystemLibs';
        ...
        this._vcpkgApplocalDeps = 'VCPKG_APPLOCAL_DEPS';
        this._vcpkgApplocalDepsInstall = 'X_VCPKG_APPLOCAL_DEPS_INSTALL';
        this._vcpkgPreferSystemLibs = 'VCPKG_PREFER_SYSTEM_LIBS';
    ...
    async runCommand(command, param, executeRoot) {
        try {
    ...
    isVcpkgEnabled() {
        ...
        let config = vscode_1.workspace.getConfiguration('vcpkg').get(this._enableVcpkgConfig);
        return config !== undefined ? config : false;
    }
    async isVcpkgExistInPath(path) {
    ...
    async installDependencies(install) {
    ...
    async onConfigurationChanged(event) {
        ...
        if (event.affectsConfiguration('vcpkg.' + this._enableVcpkgConfig)) {
            ...
            let currSel = vscode_1.workspace.getConfiguration('vcpkg').get(this._installDependenciesConfig);
            ...
            let currSel = vscode_1.workspace.getConfiguration('vcpkg').get(this._preferSystemLibsConfig);
